...
# JuliaPackage-Reuse-Audit.jl ABI/FFI Documentation
...
│  generated/abi/juliapackage-reuse-audit.h                │
└─────────────────┬───────────────────────────┘
                  │
                  │ imported by
                  ▼
...
└─────────────────┬───────────────────────────┘
                  │
                  │ compiled to libjuliapackage-reuse-audit.so/.a
                  ▼
┌─────────────────────────────────────────────┐
...
│       └── include/
│           └── juliapackage-reuse-audit.h   # C header (optional, can be generated)
│
├── generated/                  # Auto-generated files
│   └── abi/
│       └── juliapackage-reuse-audit.h       # Generated from Idris2 ABI
│
...
```bash
cd src/abi
idris2 --cg c-header Types.idr -o ../../generated/abi/juliapackage-reuse-audit.h
```
...
```c
#include "juliapackage-reuse-audit.h"

int main() {
    void* handle = juliapackage-reuse-audit_init();
    if (!handle) return 1;

    int result = juliapackage-reuse-audit_process(handle, 42);
    if (result != 0) {
        const char* err = juliapackage-reuse-audit_last_error();
...
Compile with:
```bash
gcc -o example example.c -ljuliapackage-reuse-audit -L./zig-out/lib
```

### From Idris2

```idris
import JuliaPackage-Reuse-Audit.jl.ABI.Foreign
...
```rust
#[link(name = "juliapackage-reuse-audit")]
extern "C" {
    fn juliapackage-reuse-audit_init() -> *mut std::ffi::c_void;
    fn juliapackage-reuse-audit_free(handle: *mut std::ffi::c_void);
    fn juliapackage-reuse-audit_process(handle: *mut std::ffi::c_void, input: u32) -> i32;
}

fn main() {
    unsafe {
        let handle = juliapackage-reuse-audit_init();
        assert!(!handle.is_null());

        let result = juliapackage-reuse-audit_process(handle, 42);
        assert_eq!(result, 0);
...
```julia
const libjuliapackage-reuse-audit = "libjuliapackage-reuse-audit"

function init()
    handle = ccall((:juliapackage-reuse-audit_init, libjuliapackage-reuse-audit), Ptr{Cvoid}, ())
    handle == C_NULL && error("Failed to initialize")
    handle
end

function process(handle, input)
    result = ccall((:juliapackage-reuse-audit_process, libjuliapackage-reuse-audit), Cint, (Ptr{Cvoid}, UInt32), handle, input)
    result
end

function cleanup(handle)
    ccall((:juliapackage-reuse-audit_free, libjuliapackage-reuse-audit), Cvoid, (Ptr{Cvoid},), handle)
end
...
2. **Generate C header**
   ```bash
   idris2 --cg c-header src/abi/Types.idr -o generated/abi/juliapackage-reuse-audit.h
   ```
